---
id: CANON-OSA-REGIONAL-RU16-2215-0001
title: >
  ОСА — Регион RU-16 (Набережные Челны) — структура, конвейер решений и human gate (2215)
class: canon
status: draft
version: 1.0.0
prefix: OSR
...
scope: >
  RULE-канон регионального отдела ОСА для RU-16/CHELNY.
  ...
  Локация офиса вынесена в CANON-LOC-OSA-OFFICE-CHELNY-2215-0001.
---

## LLM-INTENT

ROLE_TYPE: RULE
SCOPE: define RU-16 regional OSA structure, decision conveyor, and human gate accountability
INPUTS: [STRUCT-OSA-REGIONAL-DEPT-2215-0001, THESIS-OSA-REGIONAL-2215-0001]
...
[FACT][OSR-010] `osa_ru16_dept` = региональный отдел ОСА, ведущий кейсы и операции в RU-16/CHELNY.
[FACT][OSR-020] `osa_center` = федеральный контур ОСА (Москва), принимающий отчётность и проводящий аудит.
...
[DECISION][OSR-100] Для RU-16/CHELNY `osa_center.relation` MUST be "external_reporting_and_audit_pressure_not_ops_hq".
[DECISION][OSR-110] Для RU-16/CHELNY `osa_ru16_dept.has_flagship_team` MUST be true.
...
[FORBIDDEN][OSR-140] Подмена `osa_center` на "оперативный штаб, управляющий тактикой на земле".

## CONTENT

### 1. Center–Region Relation

[DECISION][OSR-200] `osa_center.primary_function` = "audit_and_accountability".
[DECISION][OSR-210] `osa_center.interface` MUST include {"reports","audit_requests","policy_constraints"}.

[RULE][OSR-220] IF `osa_center.requests` contains "tactical_micro_management" THEN `osa_ru16_dept.must_reject_request` = true; ELSE FAIL.

### 2. Regional Intake and Filtering

[DECISION][OSR-300] `osa_ru16_dept.intake.mode` = "continuous_signal_processing".
[DECISION][OSR-310] `osa_ru16_dept.case_rate_profile` = "rare_cases_most_incidents_not_escalated".
...
[DECISION][OSR-600] `osa_ru16_dept.flagship_team.role` = "solve_active_cases_on_ground".
[DECISION][OSR-610] `osa_ru16_dept.flagship_team.default_case_owner_role` = "case_owner".

## USAGE / RESOLUTION

[DECISION][OSR-900] Любая сцена RU-16/CHELNY с ОСА MUST трактовать регион через OSR-100..OSR-130; ELSE FAIL.
[DECISION][OSR-910] Conflict resolution MUST follow SPEC-PRIORITY-RESOLUTION-2215-0001; ELSE FAIL.

## OUTPUT CONTRACT

~~~yaml
doc_id: CANON-OSA-REGIONAL-RU16-2215-0001
role_type: RULE
export:
  - rule_id: OSR-100
    intent: define center relation for RU-16
    inputs: []
    outputs: [osa_center.relation]
  - rule_id: OSR-410
...
[FORBIDDEN][OSR-982] Представление `osa_center` как тактического командования на земле; ELSE FAIL.

## NON-NORMATIVE

~~~text
Источники: THESIS-OSA-REGIONAL-2215-0001, STRUCT-OSA-REGIONAL-DEPT-2215-0001.
